 ...
 * @author Daniela
 */
public class DespachantesImpl implements DespachantesService{
    ...
    public int obtenerIDDespachante(String empresa, String empleado){
        ...
        try{
            conec = cn.Conexion();
            pst = conec.prepareStatement("select id_despachantes from despachantes "
                    + "where empresa = '" + empresa + "' and empleado = '" + empleado + "'");
            rs = pst.executeQuery();
            
            if(rs.next()){
                IDDespachante = rs.getInt("id_despachantes");
            }
        }catch(SQLException e){
            System.err.println("No se puede obtener el ID del despachante " + e);
        }
