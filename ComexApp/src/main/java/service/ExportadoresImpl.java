...
public class ExportadoresImpl {
    ...
    public int obtenerIDExportador(String empresa, String empleado){
        ...
        try{
            conec = cn.Conexion();
            pst = conec.prepareStatement("select id_exportador from exportadores "
                    + "where empresa = '" + empresa + "' and empleado = '" + empleado + "'");
            rs = pst.executeQuery();
            
            if(rs.next()){
                IDExportador = rs.getInt("id_exportador");
            }
        }catch(SQLException e){
            System.err.println("No se puede obtener el ID del exportador " + e);
        }
          return IDExportador;      
    }
